...
class Dataset1DConditional(Dataset):
...
class GaussianDiffusion1DConditional(nn.Module):
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, cond, clip_denoised = True):
...
class Trainer1DConditional(object):
    def __init__(
        self,
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            sample_conds = torch.cat([c*torch.ones(batches[0], self.model.channels, self.model.seq_length, device=device) for c in self.sample_conds], 0)
                            ...
                            all_samples = self.ema.ema_model.sample(sample_conds, batch_size=sample_conds.shape[0])
